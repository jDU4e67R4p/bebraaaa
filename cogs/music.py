...
class QueueTooShort(commands.CommandError):
    pass


class NoPrivateMessage(commands.CommandError):
    pass
...
class MusicController:
    ...
    async def controller_loop(self):
        ...
        player = self.bot.wavelink.get_player(self.guild_id, cls=FrimePlayer)
    ...
    async def nowPlayingEmbed(self, player):
        ...
        embed = Embed(color=0x2f3136,title=f'Сейчас играет {track.title}')
...
class Music(commands.Cog):
    ...
    async def start_nodes(self):
        ...
        node = await self.bot.wavelink.initiate_node(host='178.250.156.240',
                                                     port=10008,
                                                     rest_uri='http://178.250.156.240:10008',
    ...
    async def reset_filters(self, ctx: Context):
        player = self.bot.wavelink.get_player(ctx.guild.id, cls=FrimePlayer)
    ...
    async def on_voice_state_update(self, member, before, after):
        if member.id == self.bot.user.id:
            player = self.bot.wavelink.get_player(member.guild.id, cls=FrimePlayer)
    ...
    async def teardown(self, guild_id):
        player = self.bot.wavelink.get_player(guild_id, cls=FrimePlayer)
    ...
    async def volume_prefix(self, ctx: Context, vol: int):
        if vol > 200:
            await ctx.reply("Максимальная громкость: **200**")
            return
    ...
    async def loop(self, ctx):
        ...
        if controller.loop == False:
            embed = Embed(color=0x2f3136,title=f'Трек `{player.current}` зациклен')
            controller.loop = True
            controller.toLoop = player.current
        else:
            embed = Embed(color=0x2f3136,title=f'Трек `{player.current}` больше не зациклен')
            controller.loop = False
